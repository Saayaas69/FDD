document.addEventListener("DOMContentLoaded", function () {
  ...
  const facilityDetailContent = document.getElementById(
  ...
  function closeModal(modal) {
    ...
    const id = modal.dataset.openerId;
    if (id) {
  ...
  // Wire CTAs
  if (openBookingPrimary)
    openBookingPrimary.addEventListener("click", (e) =>
      openModal(bookingModal, e.currentTarget)
    );
  if (openEnroll)
    openEnroll.addEventListener("click", (e) =>
      openModal(enrollModal, e.currentTarget)
    );
  ...
  const DETAILS = {
    gym: {
      title: "Modern Gym",
      body: "<p>Functional rigs, free weights, cardio zones and coach support.</p>",
    },
    pool: {
      title: "Olympic Pool",
      body: "<p>50m lanes, coaching lanes and timing systems.</p>",
    },
    courts: {
      title: "Indoor Courts",
      body: "<p>Multipurpose courts for basketball, volleyball and badminton.</p>",
    },
    recovery: {
      title: "Recovery Suite",
      body: "<p>Sauna, ice baths, physio rooms and recovery tech.</p>",
    },
  };
  ...
  document.querySelectorAll(".details-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      const card =
        e.currentTarget.closest(".facility-card") ||
      ...
      if (bookFrom)
        bookFrom.addEventListener(
          "click",
          () => {
            closeModal(facilityDetailsModal);
            openModal(bookingModal, bookFrom);
            setTimeout(() => {
              if (facilitySelect && key) facilitySelect.value = key;
            }, 60);
          },
          { once: true }
        );
  ...
  if (bookingForm) {
    bookingForm.addEventListener("submit", (e) => {
      ...
      if (!facility || !facility.value) {
        showFieldError(facility, "Select a facility");
        ok = false;
      }
      if (!date || !date.value) {
      ...
      if (!time || !time.value) {
        showFieldError(time, "Pick a time");
        ok = false;
      }
      if (!ok) {
      ...
      setTimeout(() => {
        ...
        showToast("Booking confirmed (demo).");
      }, 600);
    });
    bookingForm
  ...
  if (enrollForm) {
    enrollForm.addEventListener("submit", (e) => {
      ...
      if (!type || !type.value) {
        showFieldError(type, "Select a type");
        ok = false;
      }
      if (!ok) {
      ...
      setTimeout(() => {
        ...
        showToast("Enrollment submitted (demo).");
      }, 600);
    });
    enrollForm
      .querySelectorAll("input, select")
